...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for lectura_ls in laser_readings:
        d=lectura_ls[0]# distancia del el robot al obstaculo obtenida por el laser
        
        if d>0 and d<d0 and np.isfinite(d) and not np.isnan(d):
            
            theta_obs=robot_a+lectura_ls[1]#Sumamos el angulo del robot mas el obtenido por el laser respecto al obstaculo
        ...
        if i!=0:
            force_x=force_x/i   #Obtenemos el promedio
            force_y=force_y/i   #Obtenemos el promedio
...
def callback_pot_fields_goal(msg):
    ...
    while dist_to_goal>tolerance or rospy.is_shutdown():
        [fax, fay] = attraction_force(robot_x, robot_y, goal_x, goal_y)#Fuerza de atraccion
        [frx, fry] = rejection_force (robot_x, robot_y, robot_a, laser_readings)#Fuerza de repulcion.
