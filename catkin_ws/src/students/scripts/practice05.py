...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    #
    alpha = 2
    force_x = (alpha/math.sqrt((robot_x-goal_x)**2+(robot_y-goal_y)**2))*(robot_x-goal_x)
    force_y = (alpha/math.sqrt((robot_x-goal_x)**2+(robot_y-goal_y)**2))*(robot_y-goal_y)
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for lec in laser_readings:
        d = lec[0]
        
        if d>0 and d<d0 and np.isfinite(d) and not np.isnan(d):
            i += 1  
            obs_x = robot_x + d*math.cos(robot_a + lec[1])
            obs_y = robot_y + d*math.sin(robot_a + lec[1])
            force_x += ((rejec*math.sqrt((1/d) - (1/d0)))/d)*(obs_x-robot_x)
            force_y += ((rejec*math.sqrt((1/d) - (1/d0)))/d)*(obs_y-robot_y)
        else:
            pass

    if i!=0:
        [force_x, force_y] = [force_x/i, force_y/i]
