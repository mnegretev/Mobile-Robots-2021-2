...
def get_inflated_map(static_map, inflation_cells):
    ...
    for i in range(height):
        for j in range(width):
            if static_map[i,j] > 0: #Celdas con ocupadas
                for k1 in range(i-inflation_cells,i+inflation_cells +1): #Inflando renglones 
                    for k2 in range(j-inflation_cells,j+inflation_cells +1): # Inflando columnas 
                        inflated[k1,k2] = static_map[i,j] # Asignando nivel de ocupacion del mapa original
...
def get_cost_map(static_map, cost_radius):
    ...
    for i in range(height):
        for j in range(width):
            if static_map[i,j] > 0: #Para celdas ocupadas 
                for k1 in range(-cost_radius,cost_radius +1): # Renglones 
                    for k2 in range(-cost_radius,cost_radius +1): # Columnas 
                        cost = cost_radius-max(abs(k1),abs(k2)) + 1 # Calculo del costo
                        if cost_map[i+k1,j+k2] < cost: # Costo actual es mayor al costo anterior
