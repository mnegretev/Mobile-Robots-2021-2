...
def dijkstra(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    if[r,c] != [goal_r,goal_c]:
        print("Can't calculate path by Dijsktra")
        return []
    path = []
...
def get_smooth_path(original_path, alpha, beta):
    ...
    while gradient_mag > tolerance:
        ...
        for i in range(1, len(smooth_path)-1):
            ...
            [xp,yp] = smooth_path[i-1] #prev
            [xn,yn] = smooth_path[i+1] #sig
            [xo,yo] = original_path[i] 

            #Calculo gradiente
            gx = alpha*(2*xi - xp - xn) +beta*(xi - xo) 
            gy = alpha*(2*yi - yp - yn) +beta*(yi - yo)
            [xi,yi] = [xi - epsilon*gx, yi - epsilon*gy] #descenso por gradiente
            smooth_path[i] = [xi,yi]
            gradient_mag += gx**2 + gy**2 #magnitud
        
        #Ultimo punto de la serie
        [xi,yi] = smooth_path[-1] #ultimo punto
        [xp,yp] = smooth_path[-2] #penultimo
        ...
        gradient_mag += gx**2 + gy**2 #Magnitud del gradiente
