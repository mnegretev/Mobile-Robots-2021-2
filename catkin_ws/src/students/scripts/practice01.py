...
def dijkstra(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    g_values = numpy.full(grid_map.shape, sys.maxint)  #numpy.full llena un arreglo
    ...
    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        [r,c] = heapq.heappop(open_list)[1] #Obtiene nodo
        in_closed_list[r,c] = True #Se pasa a la lista cerrada
        neighbors = [[r+1,c],[r-1,c],[r,c+1],[r,c-1]]
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] != 0 or in_closed_list[nr,nc]: #Si nodo ocupado o en lista cerrada
                continue
    ...
    print("Path calculated by Dijsktra " + str(steps) + " steps")
    return path
...
def a_star(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        ...
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] != 0 or in_closed_list[nr,nc]: #Si nodo ocupado o en lista cerrada
                continue
            g = g_values[r,c] + 1 + cost_map[nr][nc]  #Calcula el valor de g
            h = numpy.abs(nr - goal_r) + numpy.abs(nc - goal_c)  #Calcula la heuristica
            f = g + h #Calcula f
