...
#define NOMBRE "ROJAS MOSQUEDA AXEL JAVIER"
     ...
     for (size_t=0,i<N;i++)
             {
                 ...
                 particles.poses[i].orientation.z=sen(theta/2);
             }
             return particles;  
...
void move_particles(geometry_msgs::PoseArray& particles, float delta_x, float delta_y, float delta_t)
{
     ...
     */
     for(size_t i=0;particles.poses.size();i++)
     {
        float a=atan2(particles.poses[i].orinetation.z,particles.poses[i].orinetation.w)*2;
        ...
        particles.poses[i].orientation.z=sen(a/2);
...
int main(int argc, char** argv)
{
    ...
    if(ros::param::has("~n"))//Para cuando deseo que el usuario nos pase los limites por parametros
