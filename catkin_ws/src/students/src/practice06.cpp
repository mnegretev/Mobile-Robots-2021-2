...
#define NOMBRE "PEREZ_VASQUEZ_CARLOS"
...
void move_particles(geometry_msgs::PoseArray& particles, float delta_x, float delta_y, float delta_t)
{
    ...
    for(size_t i=0; i<particles.poses.size(); i++)
    {
        ...
        particles.poses[i].position.x += cos(a)* delta_x- sin(a)*delta_y +rnd.gaussian(0, MOVEMENT_NOISE);
        particles.poses[i].position.y += sin(a)* delta_x+ cos(a)*delta_y + rnd.gaussian(0, MOVEMENT_NOISE);
